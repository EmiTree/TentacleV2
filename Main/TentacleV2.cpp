...
*/
ServoActuator servo(14, 15, 16, 17);
...
/*
    Super failsafe for the whole tentacle.
...
void processCommand() {
    ...
    /*
        ...
        Uppercase FORCE is checked before any normal command parsing.
    */
    if (strcmp(commandBuffer, "FORCE") == 0) {
        forceStopEverything();
        clearCommandBuffer();
        return;
    }
    ...
    int parts = sscanf(commandBuffer, "%19s %19s %f", command, subCommand, &value);

    if (parts >= 1) {
        if (strcmp(command, "pid") == 0 && parts >= 2) {
            ...
            } else if (strcmp(subCommand, "setpoint") == 0 && parts == 3) {
                setpoint = value;
                resetPid();
                printf("\nsetpoint set to %.2f\n", setpoint);
            ...
            if (strcmp(subCommand, "angle") == 0 && parts == 3) {
                servo.moveToAngle(value);
            } else if (strcmp(subCommand, "move") == 0 && parts == 3) {
                servo.moveByAngle(value);
            } else if (strcmp(subCommand, "zero") == 0) {
                servo.zeroAngleHere();
            } else if (strcmp(subCommand, "stop") == 0 || strcmp(subCommand, "0") == 0) {
                servo.stop();
            } else if (strcmp(subCommand, "status") == 0) {
                servo.printStatus();
            } else if (strcmp(subCommand, "help") == 0) {
                servo.printHelp();
            } else if (strcmp(subCommand, "s1") == 0 && parts == 3) {
                servo.setServo1Speed(value);
            } else if (strcmp(subCommand, "s2") == 0 && parts == 3) {
                servo.setServo2Speed(value);
            } else if (strcmp(subCommand, "s3") == 0 && parts == 3) {
                servo.setServo3Speed(value);
            } else if (strcmp(subCommand, "s4") == 0 && parts == 3) {
                servo.setServo4Speed(value);
            } else if (subCommand[0] == '+' || subCommand[0] == '-' || (subCommand[0] >= '0' && subCommand[0] <= '9')) {
            ...
            maxPidOutput = constrainValue(typedValue, 1.0f, 1000.0f);
            resetPid();
            printf("\nmaxPidOutput set to %.2f\n", maxPidOutput);
        } else if (strcmp(command, "motorstartpwm") == 0 && parts >= 2) {
            float typedValue = (parts == 2) ? (float)atof(subCommand) : value;
            motorStartPwm = constrainValue(typedValue, 0.0f, maxPwm);
            resetPid();
...
void printHelp() {
    ...
    printf("pid settings           -> print PID settings\n");

    printf("\nOld PID commands still work:\n");
    printf("start, stop, setpoint 0, maxpidoutput 50, motorstartpwm 20\n");
    printf("kp 5, ki 0.01, kd 0.5, settings, stopmotors\n");
